...
# --- Modelos de Datos para el Frontend (Serializables) ---
...
class State(rx.State):
    ...
    # Formulario Recogida (Pickup)
    f_pickup_address:    str = ""
    f_pickup_waste_type: str = "plástico"
    f_pickup_notes:      str = ""

    # Formularios Admin (Rutas y Puntos)
    f_route_name:  str = ""
    ...
    def _load_user_state(self, user: User):
        """Mapea los datos del modelo de DB al estado de Reflex."""
    ...
    def login(self):
        ...
        with rx.session() as db:
            ...
            # Cargar datos iniciales y redirigir
    ...
    def _load_rewards(self):
        with rx.session() as db:
            rows = db.exec(
                select(Reward).where(Reward.is_active).order_by(Reward.points_required)
            ).all()
            self.rewards = [
                RewardItem(
    ...
    def _load_ranking(self):
        with rx.session() as db:
            ...
            self.ranking = [
                RankItem(
                    rank=i + 1, id=u.id, name=u.full_name,
    ...
    def seed_if_empty(self):
        """Puebla la base de datos si las rutas están vacías."""
        with rx.session() as db:
            ...
            for nm, addr, lat, lng, types, sch, pts in puntos_init:
                db.add(CollectionPoint(
                    name=nm, address=addr, latitude=lat, longitude=lng,
            ...
            for title, desc, pts, rtype, disc, partner in rewards_init:
                db.add(Reward(
                    title=title, description=desc, points_required=pts,
